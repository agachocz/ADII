...
mobile <- read.csv("data/itu_data_1.csv") %>% filter(dataYear <= 2018, seriesUnits == "per 100 people") %>%
...
internet <- read.csv("data/internet_users.csv") %>% 
...
trpc <- read.csv("data/TRPC data protection index.txt") %>% 
...
cybersec <- read.csv("data/ITU Global Cybersecurity Index.csv") %>%
...
egov = read.csv("data/EGOV_DATA_2022.csv") %>%
...
findex <- read.csv("data/GlobalFindexDatabase2025.csv") %>% filter(group == "all") %>%
...
ease <- read.csv("data/World Bank ease of starting a business.csv", sep = ";") %>%
...
logistics_serv = read.csv("data/LPI quality of services.csv") %>%
...
logistics_infr = read.csv("data/LPI quality of infrastructure.csv") %>%
  filter(COMP_BREAKDOWN_1_LABEL == "Metric: Score", TIME_PERIOD %in% c(2016,2018,2023)) %>%
  arrange(desc(TIME_PERIOD)) %>% select(entity = REF_AREA_LABEL, OBS_VALUE) %>%
  filter(!is.na(OBS_VALUE)) %>%
  group_by(entity) %>% summarise(OBS_VALUE = first(OBS_VALUE)) %>%
...
# GCI_data is created in the file "GCI API.R", where the data is loaded using World Bank's API
...
# for some variables these countries were included in the report and not the dataset
# I used this line to check if they are present and add the values manually if not
collaboration %>% filter(entity %in% c("Singapore", "Thailand", "Vietnam", "United States"))
...
knowledge_emp <- read.csv("data/knowledge intensive employment.csv", sep = ";") %>%
...
have_id <- read.csv("data/UN id data.csv") %>% filter(str_detect(Series.Name, "ID ownership")) %>%
...
digital_id <- read.csv("data/digital id system.csv") %>% filter(str_detect(Series.Name, "Online digital identity")) %>%
  select(Country.Name, "X2021..YR2021.", "X2017..YR2017.") %>%
  pivot_longer(-Country.Name, names_to = "year", values_to = "value") %>%
  mutate(value = as.numeric(value)) %>%
...
# graduates in STEM
graduates_STEM <- read.csv("data/graduates in STEM.csv") %>% filter(year <= 2023) %>%
  group_by(geoUnit) %>%
...
# version where no missing data is allowed
clean_data <- all_data %>% drop_na()

# version where up to 3 missing indicators are allowed
small_na <- all_data$entity[rowSums(is.na(all_data)) < 4]
clean_data <- all_data %>% filter(entity %in% small_na)


# table "clean_data" is used in the file "construct the index.R" to compute ADII scores